 ...
 *
 * Design: matches current DM Auto dark/amber header.
 */
import React, {
  useCallback,
  useEffect,
  ...
  return (
    <div
      ...
      className={`group relative cursor-pointer w-full text-left flex gap-3 items-stretch px-3 py-2.5 transition-colors border-l-2 ${
        active
          ? 'bg-[#17150D] border-[var(--accent-brand)]'
          : 'bg-transparent border-transparent hover:bg-[#17150D]/60 hover:border-[var(--accent-brand)]/40'
      }`}
      data-testid={`vin-suggest-item-${item.vin || item.lot_number}`}
    >
      {/* Thumbnail */}
      <div className="w-[72px] h-[54px] rounded overflow-hidden bg-[var(--bg-base)] flex-shrink-0 flex items-center justify-center border border-[var(--border-default)]">
        {img ? (
          <img
            src={img}
            alt={title}
            ...
            onError={(e) => {
              ...
              if (p) {
                p.innerHTML =
                  '<span class="text-[9px] text-[var(--text-muted)] uppercase tracking-wider">No photo</span>';
              }
            }}
          />
        ) : (
          <span className="text-[9px] text-[var(--text-muted)] uppercase tracking-wider">No photo</span>
        )}
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center gap-0.5">
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          <span className="text-[13px] font-semibold text-[var(--text-primary)] truncate uppercase tracking-wide min-w-0">
            {title}
          </span>
          {isLive ? (
            <span
              className="inline-flex items-center gap-1 px-1.5 py-[1px] rounded-sm bg-[var(--accent-brand)]/15 text-[var(--accent-brand)] text-[8.5px] font-bold uppercase tracking-wider border border-[var(--accent-brand)]/30 flex-shrink-0"
        ...
        {/* Row 2 — VIN · LOT */}
        <div className="flex items-center gap-2 text-[10.5px] text-[var(--text-secondary)] font-mono uppercase min-w-0">
          <span className="truncate min-w-0">
            VIN: <span className="text-[var(--accent-brand)]">{item.vin || '—'}</span>
          </span>
          {item.lot_number ? (
      ...
      <div className="flex-shrink-0 flex flex-col items-end justify-between gap-1 pl-1">
        ...
        {price ? (
          <span className="text-[12.5px] font-bold text-[var(--accent-brand)] whitespace-nowrap leading-none">
            {price}
          </span>
        ) : null}
      </div>
    ...
    >
      ...
      <form
        ...
        className={`flex items-center gap-2 rounded-lg border bg-transparent px-3 h-10 transition-colors ${
          showDropdown ? 'border-[var(--accent-brand)]' : 'border-[var(--border-default)] focus-within:border-[var(--accent-brand)]'
        }`}
      >
        <button
          type="submit"
          aria-label="Search"
          className="text-[var(--text-muted)] hover:text-[var(--accent-brand)] flex-shrink-0"
          data-testid={`${testId}-submit`}
        >
          {loading ? (
            <Loader2 size={16} className="animate-spin text-[var(--accent-brand)]" />
          ) : (
            <Search size={18} />
          )}
        </button>
        <input
          ...
          className="flex-1 bg-transparent outline-none text-[14px] uppercase tracking-wide text-[var(--text-primary)] placeholder:text-[var(--text-muted)] placeholder:tracking-wide"
          autoComplete="off"
          spellCheck={false}
      ...
      {showDropdown ? (
        <div
          id={`${testId}-listbox`}
          role="listbox"
          className="absolute left-0 top-full mt-2 z-[120] w-[420px] max-w-[calc(100vw-32px)] rounded-lg border border-[var(--accent-brand)]/60 bg-[var(--bg-base)] shadow-[0_24px_48px_rgba(0,0,0,0.6)] overflow-hidden"
          data-testid={`${testId}-dropdown`}
        >
          {/* Header pill */}
          <div className="px-3 py-2 border-b border-[var(--border-default)] flex items-center justify-between bg-[#111110]">
            <span className="text-[10px] uppercase tracking-[0.2em] text-[var(--accent-brand)] font-bold flex items-center gap-2">
              {loading
                ? 'Searching…'
                : items.length > 0
              ...
              {!loading && meta.source && items.length > 0 ? (() => {
                ...
                const palette =
                  src === 'LIVE'
                    ...
                    : 'bg-[#737373]/10 text-[var(--text-muted)] border-[#737373]/30';
                const label =
          ...
          {loading && items.length === 0 ? (
            ...
            >
              <Loader2 size={14} className="animate-spin text-[var(--accent-brand)]" />
              Looking up vehicles…
            </div>
          ) : items.length > 0 ? (
            ...
            >
              ...
              <div className="text-[11px] mb-3">
                Press <span className="text-[var(--accent-brand)] font-bold">Enter</span> to run a full
                ...
                watchDone ? (
                  ...
                  >
                    ...
                    <span>
                      ...
                      <span className="font-mono text-[var(--accent-brand)]">{normalize(value)}</span> shows up.
                    </span>
                  </div>
                ) : (
                  <form
                  ...
                  >
                    <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-[var(--accent-brand)] font-semibold">
                      <Bell size={11} />
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 h-9 px-3 rounded-md bg-[#111110] border border-[var(--border-default)] text-[12px] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[var(--accent-brand)] outline-none"
                      ...
                      <button
                        ...
                        className="h-9 px-3 rounded-md bg-[var(--accent-brand)] text-[var(--text-primary)] text-[11px] font-bold uppercase tracking-wider hover:bg-[#FFC347] disabled:opacity-50 transition-colors inline-flex items-center gap-1"
