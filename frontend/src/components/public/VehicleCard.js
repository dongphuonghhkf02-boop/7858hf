    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-zinc-100">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          {isHot && (
            <span className="bg-red-500 text-[var(--text-primary)] text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1">
              <Fire size={12} weight="fill" />
              HOT
            </span>
          )}
          {auctionDate && new Date(auctionDate).getTime() - Date.now() < 24 * 60 * 60 * 1000 && (
            <span className="bg-orange-500 text-[var(--text-primary)] text-xs font-bold px-2 py-1 rounded-full flex items-center gap-1">
        ...
        {/* Lot Number */}
        {lotNumber && (
          <div className="absolute top-3 right-3 bg-[var(--bg-base)]/60 text-[var(--text-primary)] text-xs px-2 py-1 rounded">
            Лот #{lotNumber}
          </div>
        )}
      </div>
