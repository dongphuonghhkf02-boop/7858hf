...
const Label = ({ children }) => (
  <span className="text-[13px] text-[var(--text-muted)] capitalize whitespace-nowrap">{children}</span>
);
const Value = ({ children }) => (
  <span className="text-[13px] font-bold uppercase text-[var(--accent-brand)] whitespace-nowrap overflow-hidden text-ellipsis">{children || '—'}</span>
);
...
export const CarRowCard = ({ v, idx = 0 }) => {
  ...
  return (
    <div
      className="bg-[#0F0F0F] border border-[var(--border-subtle)] rounded-md overflow-hidden grid grid-cols-1 md:grid-cols-[320px_1fr] xl:grid-cols-[400px_1fr_220px] gap-5 p-5 hover:border-[var(--accent-brand)]/40 transition-colors"
    ...
    >
      ...
      <div className="flex flex-col gap-3 min-w-0">
        <Link
          ...
          className="text-[20px] md:text-[22px] xl:text-[24px] font-bold text-[var(--text-primary)] hover:text-[var(--accent-brand)] transition-colors leading-tight"
          data-testid={`car-row-${idx}-title`}
        >
          {title}
        </Link>
        <div className="flex flex-col gap-1 text-[12px] uppercase text-[var(--text-muted)]">
          <div className="flex items-center gap-2 min-w-0">
            <span className="truncate">LOT: #{lot}</span>
            <button onClick={() => copy(lot)} className="hover:text-[var(--accent-brand)] shrink-0" aria-label="Copy LOT">
              <Copy size={13} />
            </button>
          </div>
          <div className="flex items-center gap-2 min-w-0">
            ...
            <button
              onClick={() => copy(v.vin)}
              className="hover:text-[var(--accent-brand)] shrink-0"
              aria-label="Copy VIN"
            >
              <Copy size={13} />
            </button>
      ...
      <div className="flex flex-col justify-between md:col-span-2 xl:col-span-1 md:border-t xl:border-t-0 md:border-[var(--border-subtle)] md:pt-4 xl:pt-0">
        <div className="flex items-center gap-3 justify-end">
          <button
            type="button"
            className="w-8 h-8 rounded-full border border-[var(--accent-brand)] flex items-center justify-center text-[var(--accent-brand)] hover:bg-[var(--accent-brand)] hover:text-black transition-colors"
            aria-label="Compare"
        ...
        <div className="flex flex-row xl:flex-col gap-6 xl:gap-3 mt-4 xl:mt-6 flex-wrap items-start xl:items-stretch">
          <div className="min-w-[120px]">
            <div className="text-[12px] uppercase tracking-wider text-[var(--text-muted)]">Current rate</div>
            <div
              className="text-[20px] font-bold uppercase text-[var(--accent-brand)] mt-1 whitespace-nowrap"
          ...
          <div className="min-w-[120px]">
            <div className="text-[12px] uppercase tracking-wider text-[var(--text-muted)]">Auction date</div>
            <div className="text-[14px] font-bold uppercase text-[var(--text-primary)] mt-1 whitespace-nowrap">{date}</div>
          </div>
          <Link
            to={`/cars/${encodeURIComponent(id)}`}
