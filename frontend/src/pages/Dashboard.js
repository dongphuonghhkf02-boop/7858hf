...
const Dashboard = () => {
  ...
  const [topDealsPending, setTopDealsPending] = useState(0); // eslint-disable-line no-unused-vars
  ...
  const myQueueTotal = useMemo(() => {
    const vals = [myQueue.hotLeads].filter((v) => typeof v === 'number');
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[12px] text-[#71717A] mt-0.5 break-words">
              {t('updated')}: {new Date(masterData.generatedAt).toLocaleString(lang === 'ru' ? getLocale() : 'en-US')}
            </p>
          </div>
        </div>
      ...
      {/* ── My operational queue — Hot Leads only after cleanup
          (tasks + wishlist-approvals removed). */}
      ...
      {/* Top Deals approval queue widget removed — wishlist-deals manager flow disabled. */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5 mb-4 lg:mb-5">
        ...
        <div className="section-card" data-testid="sla-control">
          ...
          <div>
            ...
            <MetricRow icon={TrendUp} label={t('avgFirstResponse')} value={`${sla.avgFirstResponseMinutes} ${lang === 'ru' ? t('adm3_24a6c98c78') : 'min'}`} alert={sla.avgFirstResponseMinutes > 30} />
            ...
            {workload.managers.map((manager) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 text-xs text-[#71717A]">
                  <span>{manager.activeLeads} {lang === 'ru' ? t('adm3_194554c19c') : lang === 'ru' ? t('adm3_6d0a54f6dd') : 'leads'}</span>
                  <span>{manager.openTasks} {lang === 'ru' ? t('adm3_961b4cc3a6') : lang === 'ru' ? t('adm3_18a907b888') : 'tasks'}</span>
