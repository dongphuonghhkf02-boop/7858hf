...
const Row = ({ label, value }) => (
  <div className="grid grid-cols-[140px_1fr] gap-4 py-2.5">
    <span className="text-[13px] md:text-[14px] text-[var(--text-muted)]">{label}</span>
    <span className="text-[13px] md:text-[14px] font-semibold uppercase text-[var(--text-primary)] break-words">
...
const HighlightRow = ({ label, value }) => (
  <div className="grid grid-cols-[140px_1fr] gap-4 py-2.5">
    <span className="text-[13px] md:text-[14px] text-[var(--text-muted)]">{label}</span>
    <span className="text-[13px] md:text-[14px] font-bold uppercase text-[var(--accent-brand)] break-words">
      {value || <span className="text-[#555452]">—</span>}
    </span>
  </div>
);
...
const Chip = ({ children, tone = 'default' }) => {
  const palette =
    tone === 'amber'
      ? 'bg-[var(--accent-brand)]/10 text-[var(--accent-brand)] border-[var(--accent-brand)]/40'
      : tone === 'danger'
        ? 'bg-red-500/10 text-red-300 border-red-500/40'
        : tone === 'success'
          ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/40'
          : 'bg-white/5 text-[var(--text-primary)] border-white/15';
  return (
    <span
...
export default function VinResultPage() {
  ...
  return (
    <div data-testid="vin-result-page" className="bg-[var(--bg-base)] min-h-screen">
      <section className="pt-10 pb-8">
        <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
          ...
          >
            <div className="flex-1 flex items-center gap-2 h-[52px] px-4 border border-[var(--border-default)] focus-within:border-[var(--accent-brand)] rounded-lg bg-[#0F0F0E] transition-colors">
              <Search size={18} className="text-[var(--text-muted)]" />
              <input
                value={q}
                onChange={(e) => setQ(e.target.value)}
                placeholder="Enter VIN or LOT number"
                className="flex-1 bg-transparent outline-none text-[15px] uppercase text-[var(--text-primary)] placeholder:text-[var(--text-muted)]"
          ...
          <div className="mt-10 flex items-start justify-between gap-6 flex-wrap">
            <div>
              <div className="text-[12px] uppercase tracking-[0.2em] text-[var(--accent-brand)]">
                [ vehicle result ]
              </div>
              <h1
                className="text-[32px] md:text-[56px] font-bold text-[var(--text-primary)] leading-tight mt-3"
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <button
                ...
                className="w-10 h-10 rounded-full border border-[var(--border-default)] bg-[var(--bg-elevated)] text-[var(--accent-brand)] flex items-center justify-center hover:bg-[var(--accent-brand)] hover:text-black transition-colors disabled:opacity-50"
              ...
              <button
                ...
                className={`w-10 h-10 rounded-full border flex items-center justify-center transition-colors ${
                  inCompare
                    ? 'bg-[var(--accent-brand)] text-black border-[var(--accent-brand)]'
                    : 'border-[var(--accent-brand)] text-[var(--accent-brand)] hover:bg-[var(--accent-brand)] hover:text-black'
                } disabled:opacity-50`}
          ...
          {loading && (
            ...
            >
              <div className="h-[540px] rounded-lg bg-[var(--bg-elevated)] animate-pulse" />
              <div className="rounded-lg bg-[var(--bg-elevated)] p-8 flex items-center justify-center">
                <div className="text-center text-[var(--text-muted)]">
                  <Loader2
                    size={40}
                    className="mx-auto text-[var(--accent-brand)] animate-spin mb-4"
                  />
          ...
          {!loading && error && (
            <div
              className="mt-16 bg-[var(--bg-elevated)] rounded-lg p-10 md:p-16 text-center max-w-[900px] mx-auto"
              data-testid="vin-result-error"
            >
              <div className="w-16 h-16 rounded-full bg-[var(--accent-brand)]/10 border border-[var(--accent-brand)]/40 flex items-center justify-center mx-auto text-[var(--accent-brand)]">
                <AlertTriangle size={28} />
              </div>
              <h2 className="mt-6 text-[24px] md:text-[32px] font-bold text-[var(--text-primary)]">
                Vehicle not found
              </h2>
              <p className="mt-4 text-[14px] md:text-[15px] text-[var(--text-muted)] max-w-[620px] mx-auto">
                {error}
              </p>
              <div className="mt-8 flex items-center justify-center gap-4 flex-wrap">
                ...
                <Link
                  to="/contacts"
                  className="h-[48px] px-6 inline-flex items-center gap-2 rounded border border-[var(--accent-brand)] text-[var(--accent-brand)] text-[13px] uppercase tracking-wider font-semibold hover:bg-[var(--accent-brand)] hover:text-black transition-colors"
                ...
                <button
                  ...
                  onClick={runRescan}
                  disabled={rescanBusy}
                  className="h-[48px] px-6 inline-flex items-center gap-2 rounded border border-[var(--border-default)] text-[#E4E3DF] text-[13px] uppercase tracking-wider font-semibold hover:bg-[var(--bg-elevated)] disabled:opacity-40 transition-colors"
              ...
              <div
                className="mt-10 max-w-[560px] mx-auto p-5 rounded-xl border border-[var(--border-default)] bg-[#0F0F0E] text-left"
              ...
              >
                {watchDone ? (
                  ...
                  <form
                    ...
                    className="space-y-3"
                  >
                    <div className="flex items-center gap-2 text-[11px] uppercase tracking-wider text-[var(--accent-brand)] font-bold">
                      <Bell size={13} />
                      Notify me when VIN{' '}
                    ...
                    <div className="flex gap-2 items-stretch">
                      <input
                        ...
                        className="flex-1 h-11 px-4 rounded-md bg-[var(--bg-elevated)] border border-[var(--border-default)] text-[13px] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[var(--accent-brand)] outline-none"
                        data-testid="vin-result-watch-email"
                      />
                      <button
                        type="submit"
                        disabled={watchBusy}
                        className="h-11 px-5 rounded-md bg-[var(--accent-brand)] text-black text-[12px] font-bold uppercase tracking-wider hover:bg-[#FFC347] disabled:opacity-50 transition-colors inline-flex items-center gap-2"
          ...
          {!loading && matches.length > 1 && (
            <div
              className="mt-10 bg-[var(--bg-elevated)] rounded-lg p-6 md:p-8"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {matches.map((m) => (
                  <button
                    ...
                    className="flex gap-3 items-stretch p-3 rounded-lg border border-[var(--border-default)] bg-[#0F0F0E] hover:border-[var(--accent-brand)] transition-colors text-left group"
                  ...
                  >
                    <div className="w-24 h-20 rounded bg-[#000] overflow-hidden flex-shrink-0 flex items-center justify-center">
                      {m.image ? (
                        ...
                        <span className="text-[10px] text-[var(--text-muted)] uppercase">No image</span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-[13px] font-semibold text-[var(--text-primary)] truncate group-hover:text-[var(--accent-brand)] transition-colors">
                        {m.title || `${m.year || ''} ${m.make || ''} ${m.model || ''}`.trim() || m.vin}
                      </div>
                      <div className="text-[11px] text-[#6A6A6A] font-mono uppercase mt-1 truncate">
                        VIN: <span className="text-[var(--accent-brand)]">{m.vin}</span>
                      </div>
                      {m.lot_number ? (
                      ...
                      {m.price ? (
                        <div className="text-[12px] font-bold text-[var(--accent-brand)] mt-1">
                          {fmtMoney(m.price)}
                        </div>
                      ) : null}
                    </div>
          ...
          {!loading && v && (
            ...
            >
              ...
              <div className="bg-[var(--bg-elevated)] rounded-lg p-6 md:p-10 flex flex-col">
                <div className="flex items-start justify-between gap-4 mb-2">
                  <h2 className="text-[18px] font-semibold text-[var(--text-primary)]">Car information</h2>
                  {fmtMoney(v.price) ? (
                    <div
                      className="px-4 h-9 border border-[var(--accent-brand)] rounded flex items-center text-[15px] font-bold text-[var(--accent-brand)]"
                ...
                <div className="border-b border-[var(--border-default)] my-3" />
                ...
                <h2 className="text-[18px] font-semibold text-[var(--text-primary)] mt-8">Auction details</h2>
                <div className="border-b border-[var(--border-default)] my-3" />
                ...
                <div className="mt-6 flex items-center gap-3 flex-wrap">
                  {v.source_url ? (
                    <a
                      ...
                      className="inline-flex items-center gap-2 text-[13px] text-[var(--accent-brand)] hover:brightness-110 underline"
            ...
            <div className="rounded-2xl border border-pink-500/30 bg-gradient-to-br from-pink-950/40 via-zinc-900/80 to-zinc-900/80 p-6 md:p-8 shadow-2xl">
              <div className="flex items-start justify-between flex-wrap gap-4 mb-6">
                <div>
                  ...
                  <h2 className="text-[22px] md:text-[26px] font-bold text-[var(--text-primary)]">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
                ...
                {history.sale_date ? (
                  <div className="rounded-xl border border-zinc-700/60 bg-zinc-900/60 p-4">
                    ...
                    <div className="text-[18px] font-bold text-[var(--text-primary)] mt-1">{history.sale_date}</div>
                  </div>
                ) : null}
                {history.auction_name ? (
                  <div className="rounded-xl border border-zinc-700/60 bg-zinc-900/60 p-4">
                    <div className="text-[11px] text-zinc-400 uppercase tracking-wider font-semibold">Auction</div>
                    <div className="text-[18px] font-bold text-[var(--text-primary)] mt-1">{history.auction_name}</div>
                  </div>
                ) : null}
                {history.location ? (
                  <div className="rounded-xl border border-zinc-700/60 bg-zinc-900/60 p-4">
                    <div className="text-[11px] text-zinc-400 uppercase tracking-wider font-semibold">Location</div>
                    <div className="text-[15px] font-bold text-[var(--text-primary)] mt-1 leading-tight">{history.location}</div>
                  </div>
                ) : null}
              </div>
      ...
      {!loading && v ? (
        <section className="bg-[var(--bg-base)] pb-16" data-testid="vin-result-calc-anchor">
          <div className="max-w-[1920px] mx-auto px-6 lg:px-[100px]">
            <CarCalculator
              vehicle={{
                title,
            ...
            <div className="text-center mt-12">
              <Link
                to="/catalog"
                className="text-[14px] uppercase underline text-[var(--accent-brand)] hover:brightness-110 tracking-wider"
