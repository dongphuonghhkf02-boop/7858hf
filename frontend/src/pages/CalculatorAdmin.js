...
const CalculatorAdmin = () => {
  ...
  const loadAllData = async () => {
    setLoading(true);
    try {
      const [profileRes, statsRes, optsRes] = await Promise.all([
  ...
  // Group routes by type (Europe → destination pipeline only)
  ...
  return (
    ...
    >
      ...
      {/* ════════════════════════════════════════════════════════════ */}
      {/* EUROPE (mail.de) → BELARUS / RUSSIA pipeline                  */}
      {/* ════════════════════════════════════════════════════════════ */}
      <div className="mt-8 mb-2 flex items-center gap-3">
        <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#E4E4E7] to-transparent" />
        <h2 className="text-lg font-bold text-[#18181B] flex items-center gap-2 px-4 py-2 rounded-full bg-[#E0E7FF] border border-[#6366F1]">
          🇪🇺 Europe (mail.de) → Belarus / Russia
        </h2>
        <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#E4E4E7] to-transparent" />
      </div>

      {/* EU Inland Pickup — picks up from mail.de seller and delivers
          to the EU border crossing or export port. Bucket model
          (state/zone → port × multiplier) saved under the active
          calculator profile. */}
      <CollapsibleSection
        ...
        subtitle={
          profile?.usaInlandModel?.enabled === false
            ...
            : 'Bucket model · zone → border × multiplier'
        }
        icon={Truck}
