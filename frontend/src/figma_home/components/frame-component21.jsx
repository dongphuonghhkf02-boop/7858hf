...
const FrameComponent21 = ({ className = "", category, budget, onCount }) => {
  ...
  // Reusable DM Auto tilt-parallax for the car cards.
      ...
      <div ref={blockRef} className={`${styles.cardsBlock} tilt-scope`}>
        ...
        {isEmpty && (
          ...
          >
            <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>
              {isRu
                ? "На этой неделе пока нет предложений"
                : "No curated picks for this combo yet"}
            </div>
            <div style={{ fontSize: 14, opacity: 0.85, color: "var(--text-secondary, #51606D)", marginBottom: 18, maxWidth: 520, margin: "0 auto 18px" }}>
              {isRu
                ? "Выберите другой бюджет или категорию или просмотрите весь каталог."
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "32px 0 0", gap: 6 }}>
          <Link
            ...
            style={{
              background: "transparent", border: 0, color: "#162E51",
          ...
          >
            {loadingMore
              ...
              : (isRu ? "больше машин +" : "more vehicles +")}
          </Link>
          {total > 0 && (
            <div style={{ fontSize: 12, color: "var(--text-muted, #6E7C88)", letterSpacing: "0.04em" }}>
              {isRu
                ? `${total} подборок на этой неделе`
